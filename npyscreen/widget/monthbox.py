...
class DateEntryBase(widget.Widget):
    def __init__(self, screen, allowPastDate=True, allowTodaysDate=True, firstWeekDay=6,
                    use_datetime = False, allowClear=False, **kwargs):
        super(DateEntryBase, self).__init__(screen, **kwargs)
    ...
    def _check_today_validity(self, onErrorHigher=True):
        """
        If not allowed to select today's date, and today is selected, move
        either higher or lower depending on the value of onErrorHigher
        """
        if not self.allow_date_in_past:
            onErrorHigher = True
        if self.allow_todays_date:
...
class MonthBox(DateEntryBase):
    ...
    def __init__(self, screen, **kwargs):
        super(MonthBox, self).__init__(screen, **kwargs)

    def calculate_area_needed(self):
        #Remember that though months only have 4-5 weeks, they can span 6 weeks
    ...
    def update(self, clear=True):
        ...
        if self.value:
            ...
            if do_cal_print:
                ...
                self.add_line(self.rely + 1,
                              self.relx,
                              cal_header,
                              self.make_attributes_list(cal_header,
                                                        cal_title_attribute),
                              self.width)
            ...
            if self.do_colors():
                self.parent.curses_pad.addstr(self.rely + 9,
                                              self.relx,
                                              key_help,
